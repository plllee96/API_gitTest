...
private:
	...
	CHAR*			_fileName;
	BOOL				_isTrans;			//배경색 제거 여부
	COLORREF		_transColor;		//제거될 색상[default : MAGENTA]
...
public:
	//loadType에 따른 초기화 내용 오버로딩 필요(순서대로 빈 비트맵, 리소스, 파일)
	...
	void render(HDC hdc, int destX, int destY, int sourX, int sourY, int sourWidth, int sourHeight); //클리핑 render
